...
Louketo provides a Go programming language adapter for use with
OpenID Connect (OIDC) that supports both access tokens in a browser
cookie or bearer tokens.

This documentation details how to build and configure
Louketo Proxy followed by details of how to use each of its
features.
...
## Building
  ...
  - Run `make` to build the project. You can instead use `make static`
    if you prefer to build a binary that includes within it all of the
...
> **Note**
> 
> You can also build via docker container: `make docker-build`. A Docker
> image is available at
> <https://quay.io/louketo/louketo-proxy>.
...
## Example of usage and configuration
...
authorization headers with the `--add-claims` option. For example, a
